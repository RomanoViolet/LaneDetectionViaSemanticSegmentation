...
def verifyPythonVersion():
    assert sys.version_info >= (3, 3), "You require Python v. 3.3 or newer. You are using {a:s}".format(a=python_version())
...
def issueNoGPUWarning():
    if not tf.test.gpu_device_name():
        warnings.warn('No GPU found. Please use a GPU to train your neural network. Training on a CPU maybe 10x or more slower.')
    else:
...
def isKittiDataSetAvailable():
    if (not os.path.isdir(os.path.join(Configuration.PathtoKITTIDataSet, "data_road"))):
        print("Kitti data set is not available. Expected a folder 'data_road' at location {a:s}. Download from: {b:s}".format(a=os.path.join(Configuration.PathtoKITTIDataSet, "data_road"), b='http://www.cvlibs.net/datasets/kitti/eval_road.php'))
        sys.stdout.flush()
...
def isAugmentedVGGModelAvailable():
    if (not os.path.isfile(os.path.join(Configuration.PathtoSaveTrainedAugmentedModel, "saved_model.pb"))):
        print("Augmented VGG Model not available. Run Training first. Expected a file 'saved_model.pb' at location {a:s}".format(a=os.path.join(Configuration.PathtoSaveTrainedAugmentedModel, "saved_model.pb")))
        sys.stdout.flush()
